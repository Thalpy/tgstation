...
GLOBAL_LIST_INIT(gas_to_reagent, list(
	/datum/gas/hydrogen = /datum/reagent/gas/hydrogen,
	/datum/gas/water_vapor = /datum/reagent/water,
	/datum/gas/oxygen = /datum/reagent/gas/oxygen,
	/datum/gas/carbon_dioxide = /datum/reagent/gas/carbon_dioxide,
	/datum/gas/nitrogen = /datum/reagent/gas/nitrogen,
	/datum/gas/nitrous_oxide = /datum/reagent/gas/nitrous_oxide,
	/datum/gas/helium = /datum/reagent/gas/helium,
	/datum/gas/plasma = /datum/reagent/toxin/plasma,
	/datum/gas/bz = /datum/reagent/toxin/mindbreaker,
	/datum/gas/healium = /datum/reagent/gas/healium,
	/datum/gas/stimulum = /datum/reagent/gas/stimulum,
	/datum/gas/nitryl = /datum/reagent/gas/nitryl,
	/datum/gas/freon = /datum/reagent/gas/freon,
	/datum/gas/halon = /datum/reagent/gas/halon,
	/datum/gas/hypernoblium = /datum/reagent/gas/hypernoblium,
	/datum/gas/tritium = /datum/reagent/gas/tritium,
	/datum/gas/pluoxium = /datum/reagent/gas/pluoxium,
	/datum/gas/miasma = /datum/reagent/gas/miasma,
	/datum/gas/proto_nitrate = /datum/reagent/gas/proto_nitrate,
	/datum/gas/zauker = /datum/reagent/gas/zauker,
	/datum/gas/antinoblium = /datum/reagent/gas/antinoblium,
	))
...
/datum/reagents/proc/handle_reactions()
	...
	var/list/cached_reagents = reagent_list.Copy() //potentially change this to not work this way
...
/datum/reagents/proc/update_pressure()
	if(!length(reagent_list))
		return
	if(SEND_SIGNAL(src, COMSIG_REAGENTS_UPDATE_PRESSURE) & COMPONENT_OVERRIDE_PRESSURE_UPDATE)
		return
	if(!(flags & SEALED)) //FERMI_TODO
		...
		if(local_gas)//if we're not in nullspace
			...
			pressure = 0 //I'm assuming we're in space here
		return
	var/sum_pressure = 0
	var/sum_moles
...
/datum/reagents/proc/make_reagents_physical(turf/target_turf)
	for(var/datum/reagent/reagent as anything in reagent_list)
		for(var/datum/reagent_phase/phase_state in reagent.phase_states)
			phase_state.make_physical(reagent, reagent.get_phase_volume(phase_state.phase), target_turf)
	clear_reagents()
	...
	for(var/datum/reagent/reagent as anything in reagent_list)
		for(var/datum/reagent_phase/phase_state in reagent.phase_states)
			if(SOLID == phase_state.phase)
				...
				while(amount > 0)
					var/obj/item/stack/solid_phase_object/solid/reagent_stack = new /obj/item/stack/solid_phase_object/solid(get_turf(my_atom), amount/5, reagent)
			...
			if(POWDER == phase_state.phase)
				...
				while(amount > 0)
					var/obj/item/stack/solid_phase_object/powder/reagent_stack = new /obj/item/stack/solid_phase_object/powder(get_turf(my_atom), amount/5, reagent)
					//reagent_stack.set_reagent(reagent, min(amount, SOLID_PHYSICAL_PHASE_CAPACITY))
					amount -= SOLID_PHYSICAL_PHASE_CAPACITY
					user.put_in_hands(reagent_stack)
